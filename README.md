...
`-> [struct|class|namespace]  { /*...*/  };`  declares a code fragments.

`struct|class|namespace`  determines the type of the context a fragment can be injected in. In most cases, that type can be inferred and does not need to be specified.


`X  -> { /*...*/  };`  inject a fragment in X.  The nature of the fragment is deduced from X. X is a Type or a reflection. If X is a class, the fragment has shall not modify the memory layout of X
` -> { };`  Inject the fragment in the current class/namespace
...
The idea is I dislike the idea of functions having "code injection" as one of their side effect.
However, that does not preclude functions to return a code fragment.
...
The Shorthand notation ( which I'm a big proponent of ), allow to write
...
```
template<typename T, typename U>  requires ConceptName<T, U>
void algo(T const&);
...
When a `s-macro` expansion is encountered
 ...
 2. Each parameter is parsed,  un(qualified) names are implicitely converted to type reflexion, expression or unknown identifier depending on the expectations of the macro.
